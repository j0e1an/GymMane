...
import '../platform/pick_files.dart';
import '../platform/home_widget_api.dart';
...
class SettingsScreen extends StatelessWidget {
                ...
                child: Column(
                  children: [
                    Padding(
                      ...
                      child: Text(
                        t.signedInAs(WebSession.email ?? WebSession.name ?? 'Google'),
                        maxLines: 2,
  ...
  Future<void> _exportCsv(BuildContext context) async {
    ...
    await shareBytes(
      bytes: utf8.encode(fit.exportCsv()),
      filename: 'gymmane-workouts-$stamp.csv',
      mimeType: 'text/csv',
      subject: 'GymMane workouts',
    );
  }
  ...
  Future<void> _exportBackup(BuildContext context) async {
    ...
    await shareBytes(
      bytes: bytes,
      filename: 'gymmane-backup-$stamp.zip',
      mimeType: 'application/zip',
      subject: 'GymMane backup',
    );
  }
  ...
  Future<void> _importBackup(BuildContext context) async {
    ...
    Uint8List? bytes;
    try {
      final picked = await pickLocalFile(accept: '.zip,.json');
  ...
  Future<void> _importFromApp(BuildContext context) async {
    Uint8List? bytes;
    try {
      final picked = await pickLocalFile();
      if (picked == null) return;
      bytes = picked.bytes;
    } catch (_) {
      bytes = null;
  ...
  Future<void> _pickBackground() async {
    try {
      final picked = await pickLocalFile(accept: 'image/*');
      if (picked == null) return;
      final path = await localPathFor(path: picked.path, bytes: picked.bytes, name: picked.name);
  ...
  Future<void> _pickAlarmSound(BuildContext context) async {
    ...
    try {
      final picked = await pickLocalFile(
        accept: AlarmStore.audioExtensions.map((ext) => '.$ext').join(','),
      );
      if (picked == null) return;
      path = await localPathFor(path: picked.path, bytes: picked.bytes, name: picked.name);
      name = picked.name;
    } catch (_) {
      path = null;
    }
